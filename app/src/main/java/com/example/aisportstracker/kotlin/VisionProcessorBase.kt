...
import com.example.aisportstracker.BitmapUtils
import com.example.aisportstracker.CameraImageGraphic
import com.example.aisportstracker.FrameMetadata
import com.example.aisportstracker.GraphicOverlay
import com.example.aisportstracker.InferenceInfoGraphic
import com.example.aisportstracker.ScopedExecutor
...
abstract class VisionProcessorBase<T>(context: Context) : VisionImageProcessor {
    ...
    @RequiresApi(VERSION_CODES.LOLLIPOP)
    @ExperimentalGetImage
    override fun processImageProxyWithoutOverlay(image: ImageProxy, tvReps: TextView, tvAccuracy: TextView, bar: ProgressBar) {
        if (isShutdown) {
            return
        }
        requestDetectInImageNoOverlay(
            InputImage.fromMediaImage(image.image!!, image.imageInfo.rotationDegrees),
            tvReps,
            tvAccuracy,
            bar
    ...
    ////// My Functions
    private fun requestDetectInImageNoOverlay(
        image: InputImage,
        tvReps: TextView,
        tvAccuracy: TextView,
        bar: ProgressBar
        ...
        return task
            .addOnSuccessListener(
                ...
                OnSuccessListener { results: T ->
                    this@VisionProcessorBase.onSuccessMY(results, tvReps, tvAccuracy , bar)
                }
            )
    }
    /////
    ...
    //// My Functions
    protected abstract fun onSuccessMY(results: T, tvReps: TextView, tvAccuracy: TextView, bar: ProgressBar)
    ////
